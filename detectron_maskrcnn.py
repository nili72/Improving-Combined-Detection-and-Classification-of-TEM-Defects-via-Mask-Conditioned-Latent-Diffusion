...
'''
def get_defect_metadata(train_dataset_path, test_dataset_path, train_annotations_path, test_annotations_path):
    # Register training dataset if not already registered
    if train_dataset_path not in DatasetCatalog.list():
        DatasetCatalog.register(
            train_dataset_path,
            lambda: get_defect_anno_dict_train(train_annotations_path, train_dataset_path)
        )
        MetadataCatalog.get(train_dataset_path).set(
            thing_classes=["bdot", "111", "100"],
            thing_colors=[(0, 0, 255), (255, 0, 0), (255, 255, 0)]
        )

    defect_metadata = MetadataCatalog.get(train_dataset_path)

    # Register testing dataset if not already registered
    if test_dataset_path not in DatasetCatalog.list():
        DatasetCatalog.register(
            test_dataset_path,
            lambda: get_defect_anno_dict_val(test_annotations_path, test_dataset_path)
        )
        MetadataCatalog.get(test_dataset_path).set(
            thing_classes=["bdot", "111", "100"],
