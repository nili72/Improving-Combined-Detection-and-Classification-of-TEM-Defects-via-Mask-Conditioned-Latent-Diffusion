...
def build_dir(cfg):
    ...
    # Dictionary to store short names and their full paths
    created_folders = {}
    
    for folder in create_folders:
        folder_path = Path(folder)
        folder_path.mkdir(parents=True, exist_ok=True)  # Create the directory if it doesn't exist
        created_folders[folder_path.name] = str(folder_path)  # Map short name to full path as a string
...
def get_true_data_stats(cfg, defect_metadata, anno_dict_list_val, filename, model_checkpoint, true_and_pred_matching_threshold,
    ...
    if mask_on == True:
        if img is None:
            print(f"Error: Image {filename} could not be loaded.")
            return  # Handle the NoneType image error
        ...
        if vis is None:
            print(f"Error: Visualization failed for {filename}.")
            return  # Handle visualization failure

        img2 = vis.get_image()[:, :, ::-1]

        if save_images:
            folders = build_dir(cfg)
            pre_fig_path = Path(folders['pre_fig'])
            pre_fig_path.mkdir(parents=True, exist_ok=True)
            cv2.imwrite(os.path.join(pre_fig_path, filename + '_true_'+'TruePredMatch_'+'.png'), img2)
    ...
    if save_all_data == True:
        if mask_on == True:
            ...
            folders = build_dir(cfg)
            excel_path = Path(folders['excel'])
            excel_path.mkdir(parents=True, exist_ok=True)
            save_xls(df_list_true, os.path.join(excel_path, filename + '_TruePredMatch_'+str(true_and_pred_matching_threshold)+
...
def get_pred_data_stats(cfg, defect_metadata, anno_dict_list_val, filename, predictor, model_checkpoint,
                        true_and_pred_matching_threshold, iou_score_threshold_test, show_images=True, save_images=False,
    ...
    if save_images == True:
        ...
        pre_fig_path = Path(folders['pre_fig'])
        pre_fig_path.mkdir(parents=True, exist_ok=True)
        cv2.imwrite(os.path.join(pre_fig_path, filename + '_predicted_'+'TruePredMatch_'+
    ...
    if save_all_data == True:
        if mask_on == True:
            ...
            excel_path.mkdir(parents=True, exist_ok=True)
            print('Saving results to excel sheet...')
            save_xls(df_list, os.path.join(excel_path, filename + '_TruePredMatch_'+str(true_and_pred_matching_threshold)+
...
def get_defect_size(segmentation, image_name, defect_type):
    ...
    try:
        ...
        if area == 0.0:
            print(f"Warning: Zero area detected in ConvexHull for image {image_name}.")
            print("polygon:",polygon,"segmentation_xy:",segmentation_xy)
            defect_shape_factor = 0  # Assign default value
        else:
            radius = np.sqrt(area / np.pi)
            circumference = 2 * np.pi * radius
            defect_shape_factor = perimeter / circumference
    except Exception as e:
        print(f"Error in ConvexHull computation for image {image_name}: {e}")
        defect_shape_factor = 0  # Assign default value
...
def get_defect_number_densities(true_classes_all_flattened, pred_classes_all_flattened,
    ...
    density_bdot_percenterror = 100 * abs(pred_density_bdot - true_density_bdot) / (true_density_bdot if true_density_bdot != 0 else epsilon)
    density_111_percenterror = 100 * abs(pred_density_111 - true_density_111) / (true_density_111 if true_density_111 != 0 else epsilon)
    density_100_percenterror = 100 * abs(pred_density_100 - true_density_100) / (true_density_100 if true_density_100 != 0 else epsilon)
...
def get_overall_defect_stats(num_true_perimage, num_pred_perimage, num_found_perimage, model_checkpoint,
    ...
    # print(f"debug:num_true_total:{num_true_total}")
    ...
    overall_prec = safe_divide(num_found_total, num_found_total + overall_fp)
    overall_recall = safe_divide(num_found_total,num_found_total + overall_fn)
    overall_f1 = safe_divide(2 * overall_prec * overall_recall, overall_prec + overall_recall)
    ...
    print("overall_stats_arr:",overall_stats_arr)
...
def get_defect_sizes_average_and_errors(true_defectsizes_bdot_nm, true_defectsizes_111_nm, true_defectsizes_100_nm,
    ...
    sizes_arr = np.array([[true_defectsizes_bdot_nm, true_defectsizes_111_nm, true_defectsizes_100_nm],
                          ...
                          [true_and_pred_matching_threshold, 'n/a', 'n/a']], dtype="object")
    ...
    # print("debug, get_defect_sizes")
    # print("Lengths of the variables:")
    # print("true_defectsizes_bdot_nm length:", len(true_defectsizes_bdot_nm))
    # print("true_defectsizes_111_nm length:", len(true_defectsizes_111_nm))
    # print("true_defectsizes_100_nm length:", len(true_defectsizes_100_nm))
    # print("pred_defectsizes_bdot_nm length:", len(pred_defectsizes_bdot_nm))
    # print("pred_defectsizes_111_nm length:", len(pred_defectsizes_111_nm))
    # print("pred_defectsizes_100_nm length:", len(pred_defectsizes_100_nm))
    # print("true_defectshapes_bdot length:", len(true_defectshapes_bdot))
    # print("true_defectshapes_111 length:", len(true_defectshapes_111))
    # print("true_defectshapes_100 length:", len(true_defectshapes_100))
    # print("pred_defectshapes_bdot length:", len(pred_defectshapes_bdot))
    # print("pred_defectshapes_111 length:", len(pred_defectshapes_111))
    # print("pred_defectshapes_100 length:", len(pred_defectshapes_100))
    ...
    if not true_defectsizes_bdot_nm:
        print("Warning: true_defectsizes_bdot_nm is empty.")
        true_stats = {
            "mean": 0, "std": 0, "min": 0, 
            "25%": 0, "50%": 0, "75%": 0, "max": 0, "skew": 0
        }
    else:
        # Convert to DataFrame and compute statistics
        true_stats = pd.DataFrame(true_defectsizes_bdot_nm).describe().to_dict()[0]
    if not pred_defectsizes_bdot_nm:
        print("Warning: true_defectsizes_bdot_nm is empty.")
        pred_stats = {
            "mean": 0, "std": 0, "min": 0, 
            "25%": 0, "50%": 0, "75%": 0, "max": 0, "skew": 0
    ...
    #     print(f"{key}: {value}")
    # df_pred = pd.DataFrame(pred_defectsizes_bdot_nm)
    # if df_pred.empty:
    #     pred_stats = {"count":0,"mean": 0, "std": 0, "min": 0,"25%":0, "50%":0, "75%":0, "max": 0, "skew": 0}  # Default values or placeholders
    # else:
    #     pred_stats = df_pred.describe().to_dict()[0]
    # pred_stats = pd.DataFrame(pred_defectsizes_bdot_nm).describe().to_dict()[0]
    ...
    folders = build_dir(cfg)
    sum_path = Path(folders['sum'])
    sum_path.mkdir(parents=True, exist_ok=True)
    fig.savefig(os.path.join(sum_path, 'All_'+str(true_and_pred_matching_threshold)+
                             '_Conf_'+str(iou_score_threshold_test)+'_IoU_Blackdot_'+str(file_string)+'.png'),
    ...
    # true_stats['skew'] = true_skew
    if not true_defectsizes_111_nm:
        print("Warning: true_defectsizes_111_nm is empty.")
        true_stats = {
            "mean": 0, "std": 0, "min": 0, 
            "25%": 0, "50%": 0, "75%": 0, "max": 0, "skew": 0
        }
        ...
        # Convert to DataFrame and compute statistics
        true_stats =pd.DataFrame(true_defectsizes_111_nm).describe().to_dict()[0]
    df_pred = pd.DataFrame(pred_defectsizes_111_nm)
    if df_pred.empty:
        pred_stats = {"count":0,"mean": 0, "std": 0, "min": 0,"25%":0, "50%":0, "75%":0, "max": 0, "skew": 0}  # Default values or placeholders
    else:
        pred_stats = df_pred.describe().to_dict()[0]
    ...
    sum_path.mkdir(parents=True, exist_ok=True)
    fig.savefig(os.path.join(sum_path, 'All_'+str(true_and_pred_matching_threshold)+
                             '_Conf_'+str(iou_score_threshold_test)+'_IoU_ObjectSizes_111loops_'+str(file_string)+'.png'),
                dpi=250, bbox_inches='tight')
    ...
    # true_stats['skew'] = true_skew
    if not true_defectsizes_111_nm:
        print("Warning: true_defectsizes_100_nm is empty.")
        true_stats = {
            "mean": 0, "std": 0, "min": 0, 
            "25%": 0, "50%": 0, "75%": 0, "max": 0, "skew": 0
        }
        ...
        # Convert to DataFrame and compute statistics
        true_stats = pd.DataFrame(true_defectsizes_100_nm).describe().to_dict()[0]
    df_pred = pd.DataFrame(pred_defectsizes_100_nm)
    if df_pred.empty:
        pred_stats = {"count":0,"mean": 0, "std": 0, "min": 0,"25%":0, "50%":0, "75%":0, "max": 0, "skew": 0}  # Default values or placeholders
    else:
        pred_stats = df_pred.describe().to_dict()[0]
    ...
    sum_path.mkdir(parents=True, exist_ok=True)
    fig.savefig(os.path.join(sum_path, 'All_'+str(true_and_pred_matching_threshold)+
                             '_Conf_'+str(iou_score_threshold_test)+'_IoU_ObjectSizes_100loops_'+str(file_string)+'.png'),
                dpi=250, bbox_inches='tight')
    ...
    if not all_true_defects:
        ...
        # Convert to DataFrame and compute statistics
        true_stats = pd.DataFrame(all_true_defects).describe().to_dict()[0]
    df_pred = pd.DataFrame(all_pred_defects)  
    if df_pred.empty:
        pred_stats = {"count":0,"mean": 0, "std": 0, "min": 0,"25%":0, "50%":0, "75%":0, "max": 0, "skew": 0}  # Default values or placeholders
    else:
        pred_stats = df_pred.describe().to_dict()[0]
    ...
    sum_path.mkdir(parents=True, exist_ok=True)
    fig.savefig(os.path.join(sum_path, 'All_'+str(true_and_pred_matching_threshold)+
                             '_Conf_'+str(iou_score_threshold_test)+'_IoU_ObjectSizes_AllDefects_'+str(file_string)+'.png'),
                dpi=250, bbox_inches='tight')
    ...
    # true_stats['skew'] = true_skew
    if not true_defectshapes_bdot:
        print("Warning: true_defectshapes_bdot is empty.")
        true_stats = {
            "mean": 0, "std": 0, "min": 0, 
            "25%": 0, "50%": 0, "75%": 0, "max": 0, "skew": 0
        }
    ...
    sum_path.mkdir(parents=True, exist_ok=True)
    fig.savefig(os.path.join(sum_path, 'All_'+str(true_and_pred_matching_threshold)+
                             '_Conf_'+str(iou_score_threshold_test)+'_IoU_ObjectShapes_Blackdot_'+str(file_string)+'.png'),
                dpi=250, bbox_inches='tight')
    ...
    if not true_defectshapes_111:
        print("Warning: true_defectshapes_111 is empty.")
        true_stats = {
            "mean": 0, "std": 0, "min": 0, 
            "25%": 0, "50%": 0, "75%": 0, "max": 0, "skew": 0
        }
    ...
    sum_path.mkdir(parents=True, exist_ok=True)
    fig.savefig(os.path.join(sum_path, 'All_'+str(true_and_pred_matching_threshold)+
                             '_Conf_'+str(iou_score_threshold_test)+'_IoU_ObjectShapes_111Loops_'+str(file_string)+'.png'),
                dpi=250, bbox_inches='tight')
    ...
    # true_stats = pd.DataFrame(true_defectshapes_100).describe().to_dict()[0]
    # true_stats['skew'] = true_skew
    df_pred = pd.DataFrame(pred_defectshapes_100)
    if df_pred.empty:
        pred_stats = {"count":0,"mean": 0, "std": 0, "min": 0,"25%":0, "50%":0, "75%":0, "max": 0, "skew": 0}  # Default values or placeholders
    else:
        pred_stats = df_pred.describe().to_dict()[0]
    ...
    sum_path.mkdir(parents=True, exist_ok=True)
    fig.savefig(os.path.join(sum_path, 'All_'+str(true_and_pred_matching_threshold)+
                             '_Conf_'+str(iou_score_threshold_test)+'_IoU_ObjectShapes_100Loops_'+str(file_string)+'.png'),
                dpi=250, bbox_inches='tight')
    ...
    if not all_true_shapes:
        ...
        # Convert to DataFrame and compute statistics
        true_stats = pd.DataFrame(all_true_shapes).describe().to_dict()[0]
    df_pred = pd.DataFrame(all_pred_shapes)
    if df_pred.empty:
        pred_stats = {"count":0,"mean": 0, "std": 0, "min": 0,"25%":0, "50%":0, "75%":0, "max": 0, "skew": 0}  # Default values or placeholders
    else:
        pred_stats = df_pred.describe().to_dict()[0]
    ...
    sum_path.mkdir(parents=True, exist_ok=True)
    fig.savefig(os.path.join(sum_path, 'All_'+str(true_and_pred_matching_threshold)+
                             '_Conf_'+str(iou_score_threshold_test)+'_IoU_ObjectShapes_AllDefects_'+str(file_string)+'.png'),
                dpi=250, bbox_inches='tight')
...
def match_true_and_predicted_defects_pixelnorm_mask(true_segmentations_oneimage_abbrev, pred_segmentations_oneimage_abbrev,
        ...
        t_bdot_p_111, t_bdot_p_bdot, t_bdot_p_100,
        t_100_p_111, t_100_p_bdot, t_100_p_100,
        true_defectsizes_bdot_nm, true_defectsizes_111_nm, true_defectsizes_100_nm,
        pred_defectsizes_bdot_nm, pred_defectsizes_111_nm, pred_defectsizes_100_nm,
...
def match_true_and_predicted_defects_iou_bbox(true_classes_all_oneimage_sorted, pred_classes_all_oneimage_sorted,
    ...
    # total_true_bdot = sum([cls == 0 for cls in true_classes_all_oneimage_sorted])
    # total_true_111 = sum([cls == 1 for cls in true_classes_all_oneimage_sorted])
    # total_true_100 = sum([cls == 2 for cls in true_classes_all_oneimage_sorted])

    # total_pred_bdot = sum([cls == 0 for cls in pred_classes_all_oneimage_sorted])
    # total_pred_111 = sum([cls == 1 for cls in pred_classes_all_oneimage_sorted])
    # total_pred_100 = sum([cls == 2 for cls in pred_classes_all_oneimage_sorted])
    ...
    # calculate the Matching-based Evaluation (Like in Object Detection)
    # Match loop (same as your original)
    TP = {0: 0, 1: 0, 2: 0}
    FP = {0: 0, 1: 0, 2: 0}
    FN = {0: 0, 1: 0, 2: 0}
    ...
    for i, true_box in enumerate(true_boxes_oneimage_sorted):
        ...
        if best_iou >= iou_threshold:
            true_cls = true_classes_all_oneimage_sorted[i]
            pred_cls = pred_classes_all_oneimage_sorted[best_pred_idx]
    ...
    for j, pred_cls in enumerate(pred_classes_all_oneimage_sorted):
    ...
    for c in range(3):
        ...
        print(f"Class {c} — TP: {TP[c]}, FP: {FP[c]}, FN: {FN[c]}, Precision: {precision:.4f}, Recall: {recall:.4f}, F1: {f1:.4f}")
...
def analysis_setup(test_dataset_path, input_yaml):
    ...
    print("validation_image_filenames:",validation_image_filenames)
    
        
    # Ensure validation_image_filenames has meaningful content
    if not validation_image_filenames:
        raise ValueError(f"No files found in test dataset path: {test_dataset_path}")

    # Initialize an empty list to collect .pth files from all folders
    model_checkpoints_all = []

    # Get all model checkpoint files across the directory tree
    for _, __, files in os.walk(input_yaml['output_dir']):
        # Append .pth files from the current folder
        model_checkpoints_all.extend([f for f in files if '.pth' in f])

    print("Before filtering 'final':", model_checkpoints_all)

    # Filter out files containing 'final'
    model_checkpoints_all = [f for f in model_checkpoints_all if 'final' not in f]
    print("After filtering 'final':", model_checkpoints_all)
    ...
    if input_yaml['only_last_checkpoint'] == True:
        ...
        if not checkpoint_numbers_all:
            raise FileNotFoundError("No valid checkpoints found in output directory: {}".format(input_yaml['output_dir']))
...
def plot_overall_stats_vs_iou_threshold(save_path, full_dict_dfs_per_IoUscorethreshold):
    ...
    for iou_score_threshold_test in IoUscorethresholds:
        ...
        fig, ax = get_fig_ax(aspect_ratio=0.5, x_align=0.65)
        print("fig:",iou_score_threshold_test,TruevsPredthresholds)
    ...
    for true_and_pred_matching_threshold in TruevsPredthresholds:
        ...
        fig, ax = get_fig_ax(aspect_ratio=0.5, x_align=0.65)
        print("fig:",IoUscorethresholds,true_and_pred_matching_threshold)
...
def plot_learning_curve(cfg, classification_reports_all_checkpoints_train, classification_reports_all_checkpoints_test):
    ...
    fig.savefig(os.path.join(log_path, 'LearningCurve_Precision.png'), dpi=200, bbox_inches='tight')
    ...
    ax.legend(loc='upper right', fontsize=14, frameon=True)
    fig.savefig(os.path.join(log_path, 'LearningCurve_Recall.png'), dpi=200, bbox_inches='tight')
    ...
    ax.legend(loc='upper right', fontsize=14, frameon=True)
    fig.savefig(os.path.join(log_path, 'LearningCurve_F1score.png'), dpi=200, bbox_inches='tight')
...
def save_foundobjecttotals_per_image(save_path, image_name, num_found, total_true_objects, total_pred_objects,
    ...
    if (num_found + fp) == 0:
        print(f"Warning: Division by zero encountered. num_found={num_found}, fn={fp}")
        prec = 0  # or use None or any other default value that makes sense in your context
    else:
        prec = num_found / (num_found + fp)
    if (num_found + fn) == 0:
        print(f"Warning: Division by zero encountered. num_found={num_found}, fn={fn}")
        recall = 0  # or use None or any other default value that makes sense in your context
    else:
...
def save_objectsizes_per_image(save_path, image_name, true_classes, pred_classes, true_segs, pred_segs,
    ...
    print('1t save_objectsizes_per_image:','true_111_sizes,true_bd_sizes,true_111_sizes:',true_111_sizes,true_bd_sizes,true_100_sizes, \
    '\n pred_111_sizes,pred_bd_sizes,pred_100_sizes:',pred_111_sizes,pred_bd_sizes,pred_100_sizes)
    fig, ax = get_fig_ax(aspect_ratio=0.5, x_align=0.65)
    ...
    #             dpi=250, bbox_inches='tight')
    print('save_objectsizes_per_image:','true_111_sizes,true_bd_sizes,true_111_sizes:',true_111_sizes,true_bd_sizes,true_100_sizes, \
   '\n pred_111_sizes,pred_bd_sizes,pred_100_sizes:',pred_111_sizes,pred_bd_sizes,pred_100_sizes)
...
def init_threshold_data():
    """Initialize all counters, lists, and dicts needed for one threshold run."""
    return {
        # Basic pixel-level data
        'true_pixels_all': [],
        'pred_pixels_all': [],
        'true_classes_all': [],
        'pred_classes_all': [],

        # Defect-class matching counters
        't_bdot_p_bdot': 0, 't_111_p_bdot': 0, 't_100_p_bdot': 0,
        't_bdot_p_111': 0, 't_111_p_111': 0, 't_100_p_111': 0,
        't_bdot_p_100': 0, 't_111_p_100': 0, 't_100_p_100': 0,

        # True/pred found per image
        'num_true_perimage': [], 'num_pred_perimage': [], 'num_found_perimage': [],

        # Sizes/shapes found only
        ...
        # All sizes/shapes
        'true_defectsizes_bdot_nm_all': [], 'true_defectsizes_111_nm_all': [], 'true_defectsizes_100_nm_all': [],
        'pred_defectsizes_bdot_nm_all': [], 'pred_defectsizes_111_nm_all': [], 'pred_defectsizes_100_nm_all': [],
        'true_defectshapes_bdot_all': [], 'true_defectshapes_111_all': [], 'true_defectshapes_100_all': [],
        'pred_defectshapes_bdot_all': [], 'pred_defectshapes_111_all': [], 'pred_defectshapes_100_all': [],

        # Density/size errors, per-image
        'percent_density_error_bd_perimage_list': [],
        'percent_density_error_111_perimage_list': [],
        'percent_density_error_100_perimage_list': [],
        'percent_size_error_bd_perimage_list': [],
        'percent_size_error_111_perimage_list': [],
        'percent_size_error_100_perimage_list': [],
        'true_density_bd_perimage_list': [],
        'true_density_111_perimage_list': [],
        'true_density_100_perimage_list': [],
        'pred_density_bd_perimage_list': [],
        'pred_density_111_perimage_list': [],
        'pred_density_100_perimage_list': [],
        'true_avg_size_bd_perimage_list': [],
        'true_avg_size_111_perimage_list': [],
        'true_avg_size_100_perimage_list': [],
        'pred_avg_size_bd_perimage_list': [],
        'pred_avg_size_111_perimage_list': [],
        'pred_avg_size_100_perimage_list': [],
        'true_avg_shape_bd_perimage_list': [],
        'true_avg_shape_111_perimage_list': [],
        'true_avg_shape_100_perimage_list': [],
        'pred_avg_shape_bd_perimage_list': [],
        'pred_avg_shape_111_perimage_list': [],
        'pred_avg_shape_100_perimage_list': [],
        'image_name_list': []
    }

def maybe_load_yaml(input_yaml):
    """Load the YAML data if input_yaml is a path; otherwise return as-is if it’s already a dict."""
    if isinstance(input_yaml, dict):
        return input_yaml
    with open(input_yaml, 'r') as yaml_file:
        return yaml.safe_load(yaml_file)

def compute_final_metrics(TP_total, FP_total, FN_total):
    metrics = {}
    all_TP = all_FP = all_FN = 0

    for c in range(3):
        tp, fp, fn = TP_total[c], FP_total[c], FN_total[c]
        all_TP += tp
        all_FP += fp
        all_FN += fn
...
def match_and_compute_detection_metrics(true_classes, pred_classes,
                                        true_boxes, pred_boxes,
                                        iou_threshold=0.5):
    """
    Matches predicted and ground truth boxes using IoU and returns
    TP, FP, FN per class (0, 1, 2) for detection-style evaluation.
    Assumes all boxes are [x_min, y_min, x_max, y_max].
    """
    ...
    for i, true_box in enumerate(true_boxes):
        ...
        if best_iou >= iou_threshold:
            pred_cls = pred_classes[best_pred_idx]
            matched_pred_indices.add(best_pred_idx)
            matched_true_indices.add(i)
            if true_cls == pred_cls:
                TP[true_cls] += 1
    ...
    for c in range(3):
        metrics[c] = {
            ...
            "F1": 2 * TP[c] / (2 * TP[c] + FP[c] + FN[c]) if (2 * TP[c] + FP[c] + FN[c]) else 0.0
        }
...
def analyze_checkpoints(
    ...
    true_and_pred_matching_method='iou_bbox',
    true_and_pred_matching_thresholds=None
):
    """
    true_and_pred_matching_method (str): must be "iou_bbox" or "pixelnorm_mask"
    true_and_pred_matching_thresholds (list): thresholds for matching true & pred masks/bboxes
    """
    # Load thresholds if not provided
    if true_and_pred_matching_thresholds is None:
        try:
            yaml_data = maybe_load_yaml(input_yaml)
            true_and_pred_matching_thresholds = yaml_data.get('true_and_pred_matching_thresholds', [0.7])
            if not isinstance(true_and_pred_matching_thresholds, list):
                raise ValueError("true_and_pred_matching_thresholds must be a list.")
        except Exception as e:
            raise RuntimeError(f"Error reading true_and_pred_matching_thresholds from input_yaml: {e}")

    print(f"Using true_and_pred_matching_thresholds: {true_and_pred_matching_thresholds}")
    save_all_data = False
    save_images = True
    save_image_classification_reports = False

    # Prepare dataset and checkpoints
    validation_image_filenames, model_checkpoints, num_checkpoints, num_images = analysis_setup(
        test_dataset_path=test_dataset_path,
        input_yaml=input_yaml
    )

    classification_reports_all_checkpoints_pixels = {}
    dict_dfs = {}
    full_dict_dfs = {}
    checkpoints_done = 0
    ...
    output_path = Path(folders['excel'])
    output_path.mkdir(parents=True, exist_ok=True)
    
    ### add for save matrix data

    excel_save_path = output_path / f"match_based_metrics_{file_note}.xlsx"
    
    mode = 'a' if excel_save_path.exists() else 'w'
    ...
    # Now open once using unpacked kwargs
    with pd.ExcelWriter(**excel_writer_kwargs) as excel_writer:
        ...
        for model_checkpoint in model_checkpoints:
            ...
            if checkpoints_done < num_checkpoints:
                for true_and_pred_matching_threshold in true_and_pred_matching_thresholds:
                    ...
                    TP_total = {0: 0, 1: 0, 2: 0}
                    FP_total = {0: 0, 1: 0, 2: 0}
                    FN_total = {0: 0, 1: 0, 2: 0}
                    # Init all counters/lists
                    data = init_threshold_data()
                    data_dict_per_image = {}
                    images_done = 0
                    ...
                    for filename in validation_image_filenames:
                        if images_done >= num_images:
                            break
                        if filename not in data_dict_per_image:
                            data_dict_per_image[filename] = {}
                            # print(f"{filename} not in data_dict_per_image")
                        ...
                        # Gather true data
                        (
                            true_pixels_all_oneimage,
                            true_classes_all_oneimage,
                            true_segmentations_oneimage,
                            true_boxes_oneimage
                        ) = get_true_data_stats(
                            cfg=cfg,
                        ...
                        data['num_true_perimage'].append(len(true_boxes_oneimage))
                        # print(f"debug,num_true_perimage{filename}:,{len(true_boxes_oneimage)},{len(data['num_true_perimage'])},sum data:{np.sum(data['num_true_perimage'])},{true_boxes_oneimage}")
                        # Predict
                        try:
                            results = get_pred_data_stats(
                                cfg=cfg,
                                defect_metadata=defect_metadata,
                                anno_dict_list_val=anno_dict_list_val,
                                filename=filename,
                            ...
                            if results is None:
                                print(f"Skipping {filename}: No valid data returned from get_pred_data_stats.")
                                continue
                            (
                                pred_pixels_all_oneimage,
                                pred_classes_all_oneimage,
                                pred_segmentations_oneimage,
                                pred_boxes_oneimage
                            ) = results
                            print("SHAPES OF PRED BOXES, CLASSES, SEGS",
                                len(pred_boxes_oneimage),
                                len(pred_classes_all_oneimage),
                                len(pred_segmentations_oneimage))
                        except:
                            print("An error occurred when making predictions for image", filename)
                            continue
                        # Always record the number of true boxes for this image
                        
                        # Skip if no predictions
                        if not pred_segmentations_oneimage:
                            print("No predictions found for image", filename)
                            continue

                        if len(pred_segmentations_oneimage) != len(pred_classes_all_oneimage):
                            print("Mismatch in number of predicted segs and predicted classes.")
                            continue

                        # Sort true/pred data if needed
                        if true_segmentations_oneimage and true_classes_all_oneimage and true_boxes_oneimage:
                            (
                                true_segmentations_sorted,
                                true_classes_sorted,
                                true_boxes_sorted
                            ) = map(
                                list,
                                zip(*sorted(
                                    zip(true_segmentations_oneimage, true_classes_all_oneimage, true_boxes_oneimage)
                                ))
                            )
                            true_segmentations_abbrev = [
                                [seg[0][0], seg[1][0]] for seg in true_segmentations_sorted
                            ]
                            # print("debug:true_boxes_oneimage ", true_segmentations_oneimage, true_classes_all_oneimage, true_boxes_oneimage)
                        else:
                            print("One or more true-data lists are empty:", filename)
                            true_segmentations_sorted = []
                            true_classes_sorted = []
                            true_boxes_sorted = []
                            true_segmentations_abbrev = []
                        ...
                        (
                            pred_segmentations_sorted,
                            pred_classes_sorted,
                            pred_boxes_sorted
                        ) = map(
                            list,
                            zip(*sorted(
                                zip(pred_segmentations_oneimage, pred_classes_all_oneimage, pred_boxes_oneimage)
                            ))
                        )
                        pred_segmentations_abbrev = [
                            [seg[0][0], seg[1][0]] for seg in pred_segmentations_sorted
                        ]

                        # Match using iou_bbox or pixelnorm_mask
                        num_found = 0
                        if true_and_pred_matching_method == 'pixelnorm_mask':
                            ...
                            data['t_100_p_bdot'],
                            data['t_100_p_100'],
                            data['true_defectsizes_bdot_nm_foundonly'],
                            data['true_defectsizes_111_nm_foundonly'],
                            data['true_defectsizes_100_nm_foundonly'],
                            data['pred_defectsizes_bdot_nm_foundonly'],
                            data['pred_defectsizes_111_nm_foundonly'],
                            data['pred_defectsizes_100_nm_foundonly']
                            ) = match_true_and_predicted_defects_pixelnorm_mask(
                                true_segmentations_abbrev,
                                pred_segmentations_abbrev,
                                true_classes_sorted,
                                pred_classes_sorted,
                                ...
                                pred_segmentations_sorted,
                                num_found,
                                data['t_111_p_111'], data['t_111_p_bdot'], data['t_111_p_100'],
                                data['t_bdot_p_111'], data['t_bdot_p_bdot'], data['t_bdot_p_100'],
                                data['t_100_p_111'], data['t_100_p_bdot'], data['t_100_p_100'],
                                data['true_defectsizes_bdot_nm_foundonly'],
                                data['true_defectsizes_111_nm_foundonly'],
                                data['true_defectsizes_100_nm_foundonly'],
                                data['pred_defectsizes_bdot_nm_foundonly'],
                            ...
                            (num_found,
                            t_111_p_111_oneimage, t_111_p_bdot_oneimage, t_111_p_100_oneimage,
                            ...
                            data['pred_defectsizes_111_nm_foundonly'],
                            data['pred_defectsizes_100_nm_foundonly'],
                            data['true_defectshapes_bdot_foundonly'],
                            data['true_defectshapes_111_foundonly'],
                            data['true_defectshapes_100_foundonly'],
                            data['pred_defectshapes_bdot_foundonly'],
                            data['pred_defectshapes_111_foundonly'],
                            data['pred_defectshapes_100_foundonly']
                            ) = match_true_and_predicted_defects_iou_bbox(
                                true_classes_sorted,
                                pred_classes_sorted,
                                true_segmentations_sorted,
                                pred_segmentations_sorted,
                                ...
                                image_name=filename,
                                mask_on=input_yaml['mask_on'],
                                iou_threshold=true_and_pred_matching_threshold
                            )
                            data['t_bdot_p_bdot'] += t_bdot_p_bdot_oneimage
                            data['t_111_p_bdot'] += t_111_p_bdot_oneimage
                            data['t_100_p_bdot'] += t_100_p_bdot_oneimage
                            data['t_bdot_p_111'] += t_bdot_p_111_oneimage
                            data['t_111_p_111'] += t_111_p_111_oneimage
                            data['t_100_p_111'] += t_100_p_111_oneimage
                            data['t_bdot_p_100'] += t_bdot_p_100_oneimage
                            data['t_111_p_100'] += t_111_p_100_oneimage
                            data['t_100_p_100'] += t_100_p_100_oneimage
                        else:
                            raise ValueError("true_and_pred_matching_method must be 'pixelnorm_mask' or 'iou_bbox'")

                        print("FOUND", num_found, "CORRECT INSTANCES FOUND")
                        data['num_found_perimage'].append(num_found)
                        # data['num_true_perimage'].append(len(true_boxes_oneimage))
                        data['num_pred_perimage'].append(len(pred_boxes_oneimage))

                        # Save per-image stats
                        image_name = filename[:-4]
                        (total_true_objects, total_pred_objects) = save_objecttotals_per_image(
                        ...
                        f1 = save_foundobjecttotals_per_image(
                            save_path=cfg.OUTPUT_DIR,
                            image_name=image_name,
                            num_found=num_found,
                            total_true_objects=total_true_objects,
                        ...
                        f1_found = save_foundcorrectobjects_per_image(
                            save_path=cfg.OUTPUT_DIR,
                            image_name=image_name,
                            t_bdot_p_bdot=t_bdot_p_bdot_oneimage if 't_bdot_p_bdot_oneimage' in locals() else data['t_bdot_p_bdot'],
                            t_111_p_bdot=t_111_p_bdot_oneimage if 't_111_p_bdot_oneimage' in locals() else data['t_111_p_bdot'],
                            t_100_p_bdot=t_100_p_bdot_oneimage if 't_100_p_bdot_oneimage' in locals() else data['t_100_p_bdot'],
                            t_bdot_p_111=t_bdot_p_111_oneimage if 't_bdot_p_111_oneimage' in locals() else data['t_bdot_p_111'],
                            t_111_p_111=t_111_p_111_oneimage if 't_111_p_111_oneimage' in locals() else data['t_111_p_111'],
                            t_100_p_111=t_100_p_111_oneimage if 't_100_p_111_oneimage' in locals() else data['t_100_p_111'],
                            t_bdot_p_100=t_bdot_p_100_oneimage if 't_bdot_p_100_oneimage' in locals() else data['t_bdot_p_100'],
                            t_111_p_100=t_111_p_100_oneimage if 't_111_p_100_oneimage' in locals() else data['t_111_p_100'],
                            t_100_p_100=t_100_p_100_oneimage if 't_100_p_100_oneimage' in locals() else data['t_100_p_100'],
                            iou_score_threshold_test=iou_score_threshold_test,
                            true_and_pred_matching_threshold=true_and_pred_matching_threshold
                        )
                        ...
                        # Densities
                        densities = save_objectdensities_per_image(
                            cfg.OUTPUT_DIR,
                            image_name,
                            true_classes_all_oneimage,
                            pred_classes_all_oneimage,
                        ...
                        (pde_bd, pde_111, pde_100,
                        tdbd, td111, td100,
                        pdbd, pd111, pd100) = densities
                        data['percent_density_error_bd_perimage_list'].append(pde_bd)
                        data['percent_density_error_111_perimage_list'].append(pde_111)
                        data['percent_density_error_100_perimage_list'].append(pde_100)
                        data['true_density_bd_perimage_list'].append(tdbd)
                        data['true_density_111_perimage_list'].append(td111)
                        data['true_density_100_perimage_list'].append(td100)
                        data['pred_density_bd_perimage_list'].append(pdbd)
                        data['pred_density_111_perimage_list'].append(pd111)
                        data['pred_density_100_perimage_list'].append(pd100)

                        data_dict_per_image[filename]['true bd density'] = tdbd
                        data_dict_per_image[filename]['pred bd density'] = pdbd
                        data_dict_per_image[filename]['true 111 density'] = td111
                        data_dict_per_image[filename]['pred 111 density'] = pd111
                        data_dict_per_image[filename]['true 100 density'] = td100
                        data_dict_per_image[filename]['pred 100 density'] = pd100
                        data_dict_per_image[filename]['avg density error'] = np.mean([pde_bd, pde_111, pde_100])

                        # Sizes
                        sizes = save_objectsizes_per_image(
                            cfg.OUTPUT_DIR,
                            image_name,
                            true_classes_all_oneimage,
                            pred_classes_all_oneimage,
                        ...
                        pred_bd_sizes, pred_111_sizes, pred_100_sizes,
                        true_bd_shapes, true_111_shapes, true_100_shapes,
                        pred_bd_shapes, pred_111_shapes, pred_100_shapes) = sizes
                        data['percent_size_error_bd_perimage_list'].append(pse_bd)
                        data['percent_size_error_111_perimage_list'].append(pse_111)
                        data['percent_size_error_100_perimage_list'].append(pse_100)
                        data['true_avg_size_bd_perimage_list'].append(tasbd)
                        data['true_avg_size_111_perimage_list'].append(tas111)
                        data['true_avg_size_100_perimage_list'].append(tas100)
                        data['pred_avg_size_bd_perimage_list'].append(pasbd)
                        data['pred_avg_size_111_perimage_list'].append(pas111)
                        data['pred_avg_size_100_perimage_list'].append(pas100)
                        data['true_avg_shape_bd_perimage_list'].append(tshbd)
                        data['true_avg_shape_111_perimage_list'].append(tsh111)
                        data['true_avg_shape_100_perimage_list'].append(tsh100)
                        data['pred_avg_shape_bd_perimage_list'].append(pshbd)
                        data['pred_avg_shape_111_perimage_list'].append(psh111)
                        data['pred_avg_shape_100_perimage_list'].append(psh100)

                        data_dict_per_image[filename]['true bd avg size'] = tasbd
                        data_dict_per_image[filename]['pred bd avg size'] = pasbd
                        data_dict_per_image[filename]['true 111 avg size'] = tas111
                        data_dict_per_image[filename]['pred 111 avg size'] = pas111
                        data_dict_per_image[filename]['true 100 avg size'] = tas100
                        data_dict_per_image[filename]['pred 100 avg size'] = pas100
                        data_dict_per_image[filename]['avg size error'] = np.mean([pse_bd, pse_111, pse_100])
                        data_dict_per_image[filename]['true bd avg shape'] = tshbd
                        data_dict_per_image[filename]['pred bd avg shape'] = pshbd
                        data_dict_per_image[filename]['true 111 avg shape'] = tsh111
                        data_dict_per_image[filename]['pred 111 avg shape'] = psh111
                        data_dict_per_image[filename]['true 100 avg shape'] = tsh100
                        data_dict_per_image[filename]['pred 100 avg shape'] = psh100

                        data['true_defectsizes_bdot_nm_all'] += true_bd_sizes
                        data['true_defectsizes_111_nm_all'] += true_111_sizes
                        data['true_defectsizes_100_nm_all'] += true_100_sizes
                        data['true_defectshapes_bdot_all'] += true_bd_shapes
                        data['true_defectshapes_111_all'] += true_111_shapes
                        data['true_defectshapes_100_all'] += true_100_shapes
                        data['pred_defectsizes_bdot_nm_all'] += pred_bd_sizes
                        data['pred_defectsizes_111_nm_all'] += pred_111_sizes
                        data['pred_defectsizes_100_nm_all'] += pred_100_sizes
                        data['pred_defectshapes_bdot_all'] += pred_bd_shapes
                        data['pred_defectshapes_111_all'] += pred_111_shapes
                        data['pred_defectshapes_100_all'] += pred_100_shapes

                        # Pixel data
                        data['true_pixels_all'].append(true_pixels_all_oneimage.flatten().tolist())
                        data['pred_pixels_all'].append(pred_pixels_all_oneimage.flatten().tolist())
                        data['true_classes_all'].append(true_classes_all_oneimage)
                        data['pred_classes_all'].append(pred_classes_all_oneimage)
                        data['image_name_list'].append(filename)
                        images_done += 1
                    ################# add below for match-based classification #########
                        metrics = match_and_compute_detection_metrics(
                            true_classes_sorted,
                            pred_classes_sorted,
                            true_boxes_sorted,
                            pred_boxes_sorted,
                        ...
                        # Accumulate results
                        for cls in range(3):
                            TP_total[cls] += metrics[cls]["TP"]
                            FP_total[cls] += metrics[cls]["FP"]
                            FN_total[cls] += metrics[cls]["FN"] 
                                        
                    per_class_metrics, overall_metrics = compute_final_metrics(TP_total, FP_total, FN_total)

                    # Print results
                    print("\n--- Per-Class Metrics ---")
                    for c, m in per_class_metrics.items():
                        print(f"Class {c} — TP: {m['TP']}, FP: {m['FP']}, FN: {m['FN']}")
                        print(f"  Precision: {m['Precision']:.4f}, Recall: {m['Recall']:.4f}, F1: {m['F1']:.4f}")

                    print("\n--- Overall (Macro & Micro) Metrics ---")
                    for k, v in overall_metrics.items():
                        print(f"{k}: {v:.4f}")
                    

                    # Convert per-class metrics to DataFrame
                    df_per_class = pd.DataFrame.from_dict(per_class_metrics, orient='index')
                    df_per_class.index.name = 'Class'
                    df_per_class = df_per_class[['TP', 'FP', 'FN', 'Precision', 'Recall', 'F1']]  # enforce column order

                    # Convert overall metrics to a one-row DataFrame
                    df_overall = pd.DataFrame([overall_metrics])
                    df_overall.index = ['Overall']

                    # Concatenate for saving
                    df_save = pd.concat([df_per_class, df_overall])

                    # Define sheet name based on thresholds
                    sheet_name = f"iou{int(iou_score_threshold_test*100)}_th{int(true_and_pred_matching_threshold*100)}"
                    # Excel sheet names must be <=31 chars
                    sheet_name = sheet_name[:31]

                    # Write to Excel
                    df_save.to_excel(excel_writer, sheet_name=sheet_name)

                    # After the entire outer loop, don't forget to save the file
                    # excel_writer.close()
                    
                    ################# add above for match-based classification #########
                    
                    # Summaries
                    true_pixels_all_flattened = np.array(data['true_pixels_all']).flatten()
                    pred_pixels_all_flattened = np.array(data['pred_pixels_all']).flatten()
                    true_classes_all_flattened = list(itertools.chain(*data['true_classes_all']))
                    pred_classes_all_flattened = list(itertools.chain(*data['pred_classes_all']))

                    # Overall stats
                    df_overallstats = get_overall_defect_stats(
                        data['num_true_perimage'],
                        data['num_pred_perimage'],
                        data['num_found_perimage'],
                        model_checkpoint,
                        iou_score_threshold_test,
                        true_and_pred_matching_threshold,
                        os.path.join(
                            cfg.OUTPUT_DIR,
                            'OverallStats_' + str(len(data['image_name_list'])) + '_Images' + str(model_checkpoint)[:-4]
                    ...
                    # Confusion matrix
                    df_confusionmatrixdefectID = get_confusionmatrix_defectID(
                        t_111_p_111=data['t_111_p_111'],
                        t_111_p_bdot=data['t_111_p_bdot'],
                        t_111_p_100=data['t_111_p_100'],
                        t_bdot_p_111=data['t_bdot_p_111'],
                        t_bdot_p_bdot=data['t_bdot_p_bdot'],
                        t_bdot_p_100=data['t_bdot_p_100'],
                        t_100_p_111=data['t_100_p_111'],
                        t_100_p_bdot=data['t_100_p_bdot'],
                        t_100_p_100=data['t_100_p_100'],
                        model_checkpoint=model_checkpoint,
                        iou_score_threshold_test=iou_score_threshold_test,
                        true_and_pred_matching_threshold=true_and_pred_matching_threshold,
                        save_path=os.path.join(
                            cfg.OUTPUT_DIR,
                            'ConfusionMatrixDefectID_' + str(len(data['image_name_list']))
                            + '_Images' + str(model_checkpoint)[:-4]
                            + '_Checkpoint_' + str(iou_score_threshold_test)
                            + '_IoUPredThreshold_' + str(true_and_pred_matching_threshold)
                            + '_TruePredMatchThresh_RunType_' + str(file_note)
                        ),
                        save_to_file=False
                    )

                    # Computed stats defect ID
                    df_computedstatsdefectID = get_computedstats_defectID(
                        t_111_p_111=data['t_111_p_111'],
                        t_111_p_bdot=data['t_111_p_bdot'],
                        t_111_p_100=data['t_111_p_100'],
                        t_bdot_p_111=data['t_bdot_p_111'],
                        ...
                        save_path=os.path.join(
                            cfg.OUTPUT_DIR,
                            'ComputedStatsDefectID_' + str(len(data['image_name_list']))
                            + '_Images' + str(model_checkpoint)[:-4]
                            + '_Checkpoint_' + str(iou_score_threshold_test)
                            + '_IoUPredThreshold_' + str(true_and_pred_matching_threshold)
                            + '_TruePredMatchThresh_RunType_' + str(file_note)
                    ...
                    # Defect numbers
                    df_defectnumbers = get_defect_number_densities(
                        true_classes_all_flattened,
                        pred_classes_all_flattened,
                        num_images,
                        # len(data['image_name_list']),
                        ...
                        os.path.join(
                            cfg.OUTPUT_DIR,
                            'DefectNumbers_' + str(len(data['image_name_list'])) + '_Images' + str(model_checkpoint)[:-4]
                            + '_Checkpoint_' + str(iou_score_threshold_test)
                            + '_IoUPredThreshold_' + str(true_and_pred_matching_threshold)
                            + '_TruePredMatchThresh_RunType_' + str(file_note)
                        ),
                    ...
                    # Defect sizes (found only)
                    # print("debug df_defectsizes_FOUNDONLY")
                    # print("true_defectsizes_bdot_nm_foundonly shape:", 
                    #     data['true_defectsizes_bdot_nm_foundonly'].shape if hasattr(data['true_defectsizes_bdot_nm_foundonly'], 'shape') 
                    #     else len(data['true_defectsizes_bdot_nm_foundonly']))
                    # print("true_defectsizes_111_nm_foundonly shape:", 
                    #     data['true_defectsizes_111_nm_foundonly'].shape if hasattr(data['true_defectsizes_111_nm_foundonly'], 'shape') 
                    #     else len(data['true_defectsizes_111_nm_foundonly']))
                    # print("true_defectsizes_100_nm_foundonly shape:", 
                    #     data['true_defectsizes_100_nm_foundonly'].shape if hasattr(data['true_defectsizes_100_nm_foundonly'], 'shape') 
                    #     else len(data['true_defectsizes_100_nm_foundonly']))
                    # print("pred_defectsizes_bdot_nm_foundonly shape:", 
                    #     data['pred_defectsizes_bdot_nm_foundonly'].shape if hasattr(data['pred_defectsizes_bdot_nm_foundonly'], 'shape') 
                    #     else len(data['pred_defectsizes_bdot_nm_foundonly']))
                    # print("pred_defectsizes_111_nm_foundonly shape:", 
                    #     data['pred_defectsizes_111_nm_foundonly'].shape if hasattr(data['pred_defectsizes_111_nm_foundonly'], 'shape') 
                    #     else len(data['pred_defectsizes_111_nm_foundonly']))
                    # print("pred_defectsizes_100_nm_foundonly shape:", 
                    #     data['pred_defectsizes_100_nm_foundonly'].shape if hasattr(data['pred_defectsizes_100_nm_foundonly'], 'shape') 
                    #     else len(data['pred_defectsizes_100_nm_foundonly']))

                    
                    df_defectsizes_FOUNDONLY = get_defect_sizes_average_and_errors(
                        data['true_defectsizes_bdot_nm_foundonly'],
                        data['true_defectsizes_111_nm_foundonly'],
                        data['true_defectsizes_100_nm_foundonly'],
                        data['pred_defectsizes_bdot_nm_foundonly'],
                        ...
                        os.path.join(
                            cfg.OUTPUT_DIR,
                            'DefectSizes_FOUNDONLY' + str(len(data['image_name_list']))
                            + '_Images' + str(model_checkpoint)[:-4]
                            + '_Checkpoint_' + str(iou_score_threshold_test)
                            + '_IoUPredThreshold_' + str(true_and_pred_matching_threshold)
                            + '_TruePredMatchThresh_RunType_' + str(file_note)
                    ...
                    # Defect sizes (all)
                    df_defectsizes_ALL = get_defect_sizes_average_and_errors(
                        data['true_defectsizes_bdot_nm_all'],
                        data['true_defectsizes_111_nm_all'],
                        data['true_defectsizes_100_nm_all'],
                        data['pred_defectsizes_bdot_nm_all'],
                        data['pred_defectsizes_111_nm_all'],
                        data['pred_defectsizes_100_nm_all'],
                        data['true_defectshapes_bdot_all'],
                        data['true_defectshapes_111_all'],
                        data['true_defectshapes_100_all'],
                        data['pred_defectshapes_bdot_all'],
                        data['pred_defectshapes_111_all'],
                        data['pred_defectshapes_100_all'],
                        model_checkpoint,
                        iou_score_threshold_test,
                        true_and_pred_matching_threshold,
                        os.path.join(
                            cfg.OUTPUT_DIR,
                            'DefectSizes_ALL' + str(len(data['image_name_list']))
                            + '_Images' + str(model_checkpoint)[:-4]
                            + '_Checkpoint_' + str(iou_score_threshold_test)
                            + '_IoUPredThreshold_' + str(true_and_pred_matching_threshold)
                            + '_TruePredMatchThresh_RunType_' + str(file_note)
                    ...
                    # Make dataframes of per-image size/density errors
                    data_dict_sizes = {
                        'image names': data['image_name_list'],
                        'bd size percent error per image': data['percent_size_error_bd_perimage_list'],
                        '111 size percent error per image': data['percent_size_error_111_perimage_list'],
                        '100 size percent error per image': data['percent_size_error_100_perimage_list'],
                        'bd average size percent error per image': [np.mean(data['percent_size_error_bd_perimage_list'])],
                        'bd stdev size percent error per image': [np.std(data['percent_size_error_bd_perimage_list'])],
                        '111 average size percent error per image': [np.mean(data['percent_size_error_111_perimage_list'])],
                        '111 stdev size percent error per image': [np.std(data['percent_size_error_111_perimage_list'])],
                        '100 average size percent error per image': [np.mean(data['percent_size_error_100_perimage_list'])],
                        '100 stdev size percent error per image': [np.std(data['percent_size_error_100_perimage_list'])]
                    }
                    df_defectsizes_perimage = pd.DataFrame.from_dict(data=data_dict_sizes, orient='index')

                    data_dict_densities = {
                        'image names': data['image_name_list'],
                        'bd density percent error per image': data['percent_density_error_bd_perimage_list'],
                        '111 density percent error per image': data['percent_density_error_111_perimage_list'],
                        '100 density percent error per image': data['percent_density_error_100_perimage_list'],
                        'bd average density percent error per image': [np.mean(data['percent_density_error_bd_perimage_list'])],
                        'bd stdev density percent error per image': [np.std(data['percent_density_error_bd_perimage_list'])],
                        '111 average density percent error per image': [np.mean(data['percent_density_error_111_perimage_list'])],
                        '111 stdev density percent error per image': [np.std(data['percent_density_error_111_perimage_list'])],
                        '100 average density percent error per image': [np.mean(data['percent_density_error_100_perimage_list'])],
                        '100 stdev density percent error per image': [np.std(data['percent_density_error_100_perimage_list'])]
                    }
                    df_defectnumbers_perimage = pd.DataFrame.from_dict(data=data_dict_densities, orient='index')

                    # Collate and save
                    list_dfs = [
                        df_overallstats, df_defectsizes_FOUNDONLY, df_defectsizes_ALL, df_defectsizes_perimage,
                    ...
                    excel_path = Path(folders['excel'])
                    excel_path.mkdir(parents=True, exist_ok=True)
                    save_excel_together_singlereport(
                        list_dfs=list_dfs,
                        sheet_names=[
                            'OverallStats', 'DefectSizes_FoundOnly', 'DefectSizes_All', 'DefectSizes_PerImage',
                            'DefectNumbers', 'DefectNumbers_PerImage',
                            'ConfusionMatrixDefectID', 'ComputedStatsDefectID'
                        ],
                        save_path=os.path.join(
                            excel_path,
                            'SingleReport_' + str(len(data['image_name_list'])) + '_Images_'
                            + str(model_checkpoint)[:-4] + '_Checkpoint_' + str(iou_score_threshold_test)
                            + '_IoUPredThreshold_' + str(true_and_pred_matching_threshold)
                            + '_TruePredMatchThresh_RunType_' + str(file_note) + '.xlsx'
                        )
                    )

                    def filter_data_and_images(true_list, pred_list, image_list=None):
                        """
                        Convert lists to NumPy arrays, create a boolean mask for valid points
                        (non-NaN in both true and pred), and return the filtered arrays.
                        """
                        true_arr = np.array(true_list, dtype=float)
                        pred_arr = np.array(pred_list, dtype=float)
                        ...
                        # Single mask: only keep indices where both true & pred are not NaN
                        valid_mask = (~np.isnan(true_arr)) & (~np.isnan(pred_arr))

                        true_arr = true_arr[valid_mask]
                        pred_arr = pred_arr[valid_mask]
                        
                        if img_arr is not None:
                            img_arr = img_arr[valid_mask]
                            return true_arr, pred_arr, img_arr
                        else:
                            return true_arr, pred_arr

                    # ---------------------------------------------------------
                    # Inside your large code block after data is collected:
                    # ---------------------------------------------------------
                    ...
                    true_bd_array, pred_bd_array, img_bd_array = filter_data_and_images(
                        data['true_avg_size_bd_perimage_list'],
                        data['pred_avg_size_bd_perimage_list'],
                        data['image_name_list']
                    )

                    if len(true_bd_array) > 0:
                        ax.scatter(true_bd_array, pred_bd_array,
                                color='blue', edgecolors='black', s=100, zorder=2, alpha=0.7)
                        ...
                        # Axis scaling
                        xlow = int(min(true_bd_array) - 0.1 * (max(true_bd_array) - min(true_bd_array)))
                        xhigh = int(max(true_bd_array) + 0.1 * (max(true_bd_array) - min(true_bd_array)))
                        ...
                        # Metrics
                        r2 = r2_score(true_bd_array, pred_bd_array)
                        mae = mean_absolute_error(true_bd_array, pred_bd_array)
                        rmse = np.sqrt(mean_squared_error(true_bd_array, pred_bd_array))
                    ...
                    fig.savefig(
                        os.path.join(
                            sum_path,
                            f"DefectSize_ParityPlot_TruePredMatch_{true_and_pred_matching_threshold}"
                            f"_IoUScoreThresh_{iou_score_threshold_test}_Blackdot.png"
                        ),
                        dpi=250, bbox_inches='tight'
                    )
                    ...
                    df_plot = pd.DataFrame({
                        'image name': img_bd_array,
                        'true bd size': true_bd_array,
                        'pred bd size': pred_bd_array,
                    })
                    df_plot.to_excel(
                        os.path.join(
                            sum_path,
                            f"DefectSize_ParityPlot_TruePredMatch_{true_and_pred_matching_threshold}"
                            f"_IoUScoreThresh_{iou_score_threshold_test}_Blackdot.xlsx"
                        ),
                        index=False
                    )
                    ...
                    true_111_array, pred_111_array, img_111_array = filter_data_and_images(
                        data['true_avg_size_111_perimage_list'],
                        data['pred_avg_size_111_perimage_list'],
                        data['image_name_list']
                    )

                    if len(true_111_array) > 0:
                        ax.scatter(true_111_array, pred_111_array,
                                color='red', edgecolors='black', s=100, zorder=2, alpha=0.7)
                        ax.set_xlabel('True avg <111> sizes per image (nm)', fontsize=12)
                        ax.set_ylabel('Predicted avg <111> sizes per image (nm)', fontsize=12)

                        xlow = int(min(true_111_array) - 0.1 * (max(true_111_array) - min(true_111_array)))
                        xhigh = int(max(true_111_array) + 0.1 * (max(true_111_array) - min(true_111_array)))
                        ax.set_xlim(left=xlow, right=xhigh)
                        ax.set_ylim(bottom=xlow, top=xhigh)
                        ax.plot([xlow, xhigh], [xlow, xhigh], color='black', lw=1.5, linestyle='--')

                        r2 = r2_score(true_111_array, pred_111_array)
                        mae = mean_absolute_error(true_111_array, pred_111_array)
                        rmse = np.sqrt(mean_squared_error(true_111_array, pred_111_array))
                    ...
                    fig.savefig(
                        os.path.join(
                            sum_path,
                            f"DefectSize_ParityPlot_TruePredMatch_{true_and_pred_matching_threshold}"
                            f"_IoUScoreThresh_{iou_score_threshold_test}_111Loop.png"
                        ),
                        dpi=250, bbox_inches='tight'
                    )

                    df_plot = pd.DataFrame({
                        'image name': img_111_array,
                        'true 111 size': true_111_array,
                        'pred 111 size': pred_111_array,
                    })
                    df_plot.to_excel(
                        os.path.join(
                            sum_path,
                            f"DefectSize_ParityPlot_TruePredMatch_{true_and_pred_matching_threshold}"
                            f"_IoUScoreThresh_{iou_score_threshold_test}_111Loop.xlsx"
                        ),
                        index=False
                    )
                    ...
                    true_100_array, pred_100_array, img_100_array = filter_data_and_images(
                        data['true_avg_size_100_perimage_list'],
                        data['pred_avg_size_100_perimage_list'],
                        data['image_name_list']
                    )

                    if len(true_100_array) > 0:
                        ax.scatter(true_100_array, pred_100_array,
                                color='yellow', edgecolors='black', s=100, zorder=2, alpha=0.7)
                        ax.set_xlabel('True avg <100> sizes per image (nm)', fontsize=12)
                        ax.set_ylabel('Predicted avg <100> sizes per image (nm)', fontsize=12)

                        xlow = int(min(true_100_array) - 0.1 * (max(true_100_array) - min(true_100_array)))
                        xhigh = int(max(true_100_array) + 0.1 * (max(true_100_array) - min(true_100_array)))
                        ax.set_xlim(left=xlow, right=xhigh)
                        ax.set_ylim(bottom=xlow, top=xhigh)
                        ax.plot([xlow, xhigh], [xlow, xhigh], color='black', lw=1.5, linestyle='--')

                        r2 = r2_score(true_100_array, pred_100_array)
                        mae = mean_absolute_error(true_100_array, pred_100_array)
                        rmse = np.sqrt(mean_squared_error(true_100_array, pred_100_array))
                        d = {'R2': r2, 'MAE': mae, 'root_mean_squared_error': rmse}
                        plot_stats(fig, d, x_align=0.15, y_align=0.90, type='float')
                    ...
                    fig.savefig(
                        os.path.join(
                            sum_path,
                            f"DefectSize_ParityPlot_TruePredMatch_{true_and_pred_matching_threshold}"
                            f"_IoUScoreThresh_{iou_score_threshold_test}_100Loop.png"
                        ),
                        dpi=250, bbox_inches='tight'
                    )

                    df_plot = pd.DataFrame({
                        'image name': img_100_array,
                        'true 100 size': true_100_array,
                        'pred 100 size': pred_100_array,
                    })
                    df_plot.to_excel(
                        os.path.join(
                            sum_path,
                            f"DefectSize_ParityPlot_TruePredMatch_{true_and_pred_matching_threshold}"
                            f"_IoUScoreThresh_{iou_score_threshold_test}_100Loop.xlsx"
                        ),
                        index=False
                    )
                    ...
                    ############
                    # Combine arrays for 'all true' and 'all pred'
                    all_true_sizes = list(true_100_array) + list(true_111_array) + list(true_bd_array)
                    all_pred_sizes = list(pred_100_array) + list(pred_111_array) + list(pred_bd_array)

                    fig, ax = get_fig_ax(aspect_ratio=0.5, x_align=0.65)
                    ax.scatter(true_bd_array, pred_bd_array, color='blue', edgecolors='black', s=100, alpha=0.7, label='black dot')
                    ax.scatter(true_111_array, pred_111_array, color='red', edgecolors='black', s=100, alpha=0.7, label='<111> loop')
                    ax.scatter(true_100_array, pred_100_array, color='yellow', edgecolors='black', s=100, alpha=0.7, label='<100> loop')
                    ax.legend(loc='lower right')
                    ...
                    if len(all_true_sizes) > 0:
                        xlow = int(min(all_true_sizes) - 0.1*(max(all_true_sizes) - min(all_true_sizes)))
                        xhigh = int(max(all_true_sizes) + 0.1*(max(all_true_sizes) - min(all_true_sizes)))
                        ax.set_xlim(left=xlow, right=xhigh)
                        ax.set_ylim(bottom=xlow, top=xhigh)
                        ax.plot([xlow, xhigh], [xlow, xhigh], color='black', lw=1.5, linestyle='--')

                        r2 = r2_score(all_true_sizes, all_pred_sizes)
                        mae = mean_absolute_error(all_true_sizes, all_pred_sizes)
                        rmse = np.sqrt(mean_squared_error(all_true_sizes, all_pred_sizes))
                        d = {'R2': r2, 'MAE': mae, 'root_mean_squared_error': rmse}
                        plot_stats(fig, d, x_align=0.15, y_align=0.90, type='float')
                    ...
                    fig.savefig(
                        os.path.join(
                            sum_path,
                            f"DefectSize_ParityPlot_TruePredMatch_{true_and_pred_matching_threshold}"
                            f"_IoUScoreThresh_{iou_score_threshold_test}_AllDefects.png"
                        ),
                        dpi=250, bbox_inches='tight'
                    )
                    ...
                    ############
                    # ALL DEFECT SIZES AVG & STDEV
                    ############
                    # (No filtering needed here, because you're computing means of the *already filtered data*.)

                    true_sizes_100_avg = np.mean(true_100_array) if len(true_100_array) > 0 else np.nan
                    true_sizes_100_std = np.std(true_100_array)  if len(true_100_array) > 0 else np.nan
                    pred_sizes_100_avg = np.mean(pred_100_array) if len(pred_100_array) > 0 else np.nan
                    pred_sizes_100_std = np.std(pred_100_array)  if len(pred_100_array) > 0 else np.nan

                    true_sizes_111_avg = np.mean(true_111_array) if len(true_111_array) > 0 else np.nan
                    true_sizes_111_std = np.std(true_111_array)  if len(true_111_array) > 0 else np.nan
                    pred_sizes_111_avg = np.mean(pred_111_array) if len(pred_111_array) > 0 else np.nan
                    pred_sizes_111_std = np.std(pred_111_array)  if len(pred_111_array) > 0 else np.nan

                    true_sizes_bd_avg = np.mean(true_bd_array)   if len(true_bd_array) > 0 else np.nan
                    true_sizes_bd_std = np.std(true_bd_array)    if len(true_bd_array) > 0 else np.nan
                    pred_sizes_bd_avg = np.mean(pred_bd_array)   if len(pred_bd_array) > 0 else np.nan
                    pred_sizes_bd_std = np.std(pred_bd_array)    if len(pred_bd_array) > 0 else np.nan

                    fig, ax = get_fig_ax(aspect_ratio=0.5, x_align=0.65)
                    # black dot
                    ax.scatter(true_sizes_bd_avg, pred_sizes_bd_avg, color='blue', edgecolors='black', s=100, alpha=0.7,
                            label='black dot average')
                    ax.errorbar(true_sizes_bd_avg, pred_sizes_bd_avg,
                                xerr=true_sizes_bd_std, yerr=pred_sizes_bd_std,
                                capsize=2, ecolor='k', linestyle='none')

                    # 111
                    ax.scatter(true_sizes_111_avg, pred_sizes_111_avg, color='red', edgecolors='black', s=100, alpha=0.7,
                            label='<111> loop average')
                    ax.errorbar(true_sizes_111_avg, pred_sizes_111_avg,
                                xerr=true_sizes_111_std, yerr=pred_sizes_111_std,
                                capsize=2, ecolor='k', linestyle='none')

                    # 100
                    ax.scatter(true_sizes_100_avg, pred_sizes_100_avg, color='yellow', edgecolors='black', s=100, alpha=0.7,
                            label='<100> loop average')
                    ax.errorbar(true_sizes_100_avg, pred_sizes_100_avg,
                                xerr=true_sizes_100_std, yerr=pred_sizes_100_std,
                                capsize=2, ecolor='k', linestyle='none')
                    ...
                    # If all three are NaN, it won't plot well. But let's do a quick check:
                    combined_avg_true = [true_sizes_100_avg, true_sizes_111_avg, true_sizes_bd_avg]
                    combined_avg_pred = [pred_sizes_100_avg, pred_sizes_111_avg, pred_sizes_bd_avg]

                    valid_for_all = [not np.isnan(x) for x in (combined_avg_true + combined_avg_pred)]
                    if any(valid_for_all):
                        # Filter out the nans for axis-limits:
                        combined_all_true = [t for t in combined_avg_true if not np.isnan(t)]
                        if len(combined_all_true) > 0:
                            xlow = int(min(combined_all_true) - 0.1 * (max(combined_all_true) - min(combined_all_true)))
                            xhigh = int(max(combined_all_true) + 0.1 * (max(combined_all_true) - min(combined_all_true)))
                            ax.set_xlim(left=xlow, right=xhigh)
                            ax.set_ylim(bottom=xlow, top=xhigh)
                            ax.plot([xlow, xhigh], [xlow, xhigh], color='black', lw=1.5, linestyle='--')

                        # Metrics using just [100,111,bd] means
                        # Filter out nans from each
                        X_t = [val for val in combined_avg_true if not np.isnan(val)]
                        X_p = [val for val in combined_avg_pred if not np.isnan(val)]
                        if len(X_t) == len(X_p) and len(X_t) > 0:
                            r2  = r2_score(X_t, X_p)
                            mae = mean_absolute_error(X_t, X_p)
                            rmse = np.sqrt(mean_squared_error(X_t, X_p))
                            d = {'R2': r2, 'MAE': mae, 'RMSE': rmse}
                            plot_stats(fig, d, x_align=0.15, y_align=0.90, type='float')
                    ...
                    fig.savefig(
                        os.path.join(
                            sum_path,
                            f"DefectSize_AvgStdev_ParityPlot_TruePredMatch_{true_and_pred_matching_threshold}"
                            f"_IoUScoreThresh_{iou_score_threshold_test}_AllDefects.png"
                        ),
                        dpi=250, bbox_inches='tight'
                    )
                    ...
                    ############################################################
                    # REPEAT SIMILAR SINGLE-PASS FILTERING FOR DENSITIES & SHAPES
                    ############################################################

                    # For example, black dot densities:
                    fig, ax = get_fig_ax(aspect_ratio=0.5, x_align=0.65)

                    true_bd_dens, pred_bd_dens, img_bd_dens = filter_data_and_images(
                        data['true_density_bd_perimage_list'],
                        data['pred_density_bd_perimage_list'],
                        data['image_name_list']
                    )

                    # Multiply by 1e4 if needed:
                    true_bd_dens *= 1e4
                    pred_bd_dens *= 1e4

                    if len(true_bd_dens) > 0:
                        ax.scatter(true_bd_dens, pred_bd_dens,
                                color='blue', edgecolors='black', s=100, alpha=0.7)
                        ax.set_xlabel('True black dot densities (x10$^4$ #/nm$^2$)', fontsize=12)
                        ax.set_ylabel('Predicted black dot densities (x10$^4$ #/nm$^2$)', fontsize=12)
                        xlow = int(min(true_bd_dens) - 0.1*(max(true_bd_dens) - min(true_bd_dens)))
                        xhigh = int(max(true_bd_dens) + 0.1*(max(true_bd_dens) - min(true_bd_dens)))
                        ax.set_xlim(left=xlow, right=xhigh)
                        ax.set_ylim(bottom=xlow, top=xhigh)
                        ax.plot([xlow, xhigh], [xlow, xhigh], color='black', lw=1.5, linestyle='--')

                        # Metrics
                        r2 = r2_score(true_bd_dens, pred_bd_dens)
                        mae = mean_absolute_error(true_bd_dens, pred_bd_dens)
                        rmse = np.sqrt(mean_squared_error(true_bd_dens, pred_bd_dens))
                        d = {'R2': r2, 'MAE': mae, 'RMSE': rmse}
                        plot_stats(fig, d, x_align=0.15, y_align=0.90, type='float')
                    ...
                    fig.savefig(
                        os.path.join(
                            sum_path,
                            f"DefectDensity_ParityPlot_TruePredMatch_{true_and_pred_matching_threshold}"
                            f"_IoUScoreThresh_{iou_score_threshold_test}_Blackdot.png"
                        ),
                        dpi=250, bbox_inches='tight'
                    )
                    ...
                    df_plot = pd.DataFrame({
                        'image name': img_bd_dens,
                        'true bd density': true_bd_dens,
                        'pred bd density': pred_bd_dens
                    })
                    df_plot.to_excel(
                        os.path.join(
                            sum_path,
                    ...
                    # ... and similarly for 111 densities, 100 densities, shapes, etc.
                    # Each time:
                    #   1) filter_data_and_images(...)
                    #   2) scatter
                    #   3) set axis
                    #   4) do metrics
                    #   5) save figure & excel
                    ...
                    # -----------------------------------------------------------
                    # AFTER ALL PLOTS:
                    # SAVE data_dict_per_image TO JSON
                    # -----------------------------------------------------------
                    with open(
                        os.path.join(
                            sum_path,
                            f"StatsPerImage_TruePredMatch_{true_and_pred_matching_threshold}"
                            f"_IoUScoreThresh_{iou_score_threshold_test}.json"
                        ),
                        'w'
                    ) as f:
                    ...
                    # Identify best/worst images
                    data_dict_best_images = {}
                    data_dict_worst_images = {}
                    best_f1 = -float('inf')
                    best_find_f1 = 0
                    best_density = 1e5
                    best_size = 1e5
                    ...
                    for img, val in data_dict_per_image.items():
                        asize = val.get('avg size error', float('inf'))
                        adensity = val.get('avg density error', float('inf'))
                        of1 = val.get('overall F1', -float('inf'))
                        df1 = val.get('defect find F1', -float('inf'))

                        # Best
                        if asize < best_size:
                            best_size = asize
                            data_dict_best_images['best avg size error'] = img
                        if adensity < best_density:
                            best_density = adensity
                        ...
                        if adensity > worst_density:
                            worst_density = adensity
                            data_dict_worst_images['worst avg density error'] = img
                        if of1 < worst_f1:
                            worst_f1 = of1
                            data_dict_worst_images['worst overall F1'] = img
                        if df1 < worst_find_f1:
                            worst_find_f1 = df1
                            data_dict_worst_images['worst defect find F1'] = img

                    with open(os.path.join(cfg.OUTPUT_DIR,
                                        f'BestImagesPerStat_TruePredMatch_{true_and_pred_matching_threshold}'
                                        f'_IoUScoreThresh_{iou_score_threshold_test}.json'), 'w') as f:
                        json.dump(data_dict_best_images, f)

                    with open(os.path.join(cfg.OUTPUT_DIR,
                                        f'WorstImagesPerStat_TruePredMatch_{true_and_pred_matching_threshold}'
                                        f'_IoUScoreThresh_{iou_score_threshold_test}.json'), 'w') as f:
