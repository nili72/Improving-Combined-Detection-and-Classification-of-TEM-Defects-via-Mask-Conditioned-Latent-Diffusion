...
wandb.login(key="4dfbb4e0161bd81ab09e20e8beee3b7ecfe457ec")
...
class WandbWriter(EventWriter):
    ...
    def write(self):
        """
        Called by Detectron2 (via PeriodicWriter) to write out the latest metrics.
        """
        # Retrieve the current storage
        storage = get_event_storage()

        # Log the latest metrics to wandb
...
class WandbTrainer(DefaultTrainer):
    def build_writers(self):
        """
        Override the build_writers method to add WandbWriter.
        """
        writers = super().build_writers()  # Get default writers (Tensorboard, etc.)
        writers.append(WandbWriter())      # Add our custom WandbWriter
        return writers
# class WandbWriter(EventWriter):
#     def __init__(self):
#         super().__init__()  # Initialize the EventWriter

#     def write(self):
#         # Access the EventStorage
#         storage = self.storage
#         # Log all the latest metrics to wandb
#         for key, value in storage.latest().items():
#             if isinstance(value, (int, float)):  # Log only numerical metrics
#                 wandb.log({key: value})
...
#         Override the build_writers method to add WandbWriter.
#         """
#         writers = super().build_writers()  # Get default writers
#         writers.append(WandbWriter())  # Add custom WandbWriter
#         return writers
...
def get_config_file(input_yaml):
    ...
    if input_yaml['mask_on'] == True:
        if input_yaml['cascade_maskrcnn'] == True:
            ...
            cfg.merge_from_file("/ocean/projects/mat240020p/nli1/diffusion/nuohao/code/maskrcnn_for_Ni/detectron2_configs/configs/COCO-InstanceSegmentation/"+str(input_yaml['pretrained_model_name']))
    ...
    model_weights_urls = {"cascade_mask_rcnn_R_50_FPN_1x": "/srv/home/rjacobs3/anaconda3/envs/testenv2/lib/python3.6/site-packages/detectron2_repo/configs/Misc/model_weights/cascade_mask_rcnn_R_50_FPN_1x.pkl",
                        ...
                        "mask_rcnn_R_50_FPN_3x": "/ocean/projects/mat240020p/nliu3/maskrcnn/code/libs/weights/COCO-InstanceSegmentation/model_weights/mask_rcnn_R_50_FPN_3x_model_final_f10217.pkl",
    ...
    # Initialize wandb
    wandb.init(
        project="maskrcnn",  # Replace with your wandb project name
        # entity="knowhow07-university-of-wisconsin-madison-org",  # Optional: your wandb username or organization
        config={"learning_rate": cfg.SOLVER.BASE_LR, 
                "batch_size": cfg.MODEL.ROI_HEADS.BATCH_SIZE_PER_IMAGE,
                "true_and_pred_matching_thresholds": input_yaml['true_and_pred_matching_thresholds'],
                "iou_score_threshold_test":input_yaml['iou_score_threshold_test'],
                "roi_iou_threshold": cfg.MODEL.ROI_HEADS.IOU_THRESHOLDS },  # Add any hyperparameters here,
...
def make_trainer(cfg, starting_fresh=True):
    ...
    if starting_fresh == True:
        ...
        trainer = WandbTrainer(cfg)  # Use the custom WandbTrainer
