...
#sys.path.insert(0, 'fvcore')
#sys.path.insert(0, 'mast_defectdetection')
#sys.path.insert(0, 'tabulate')
#sys.path.insert(0, 'detectron_required_packages')
print('sys path')
print(sys.path)
import yaml
import argparse
import os
import shutil
from detectron_maskrcnn import train_detectron_maskrcnn, get_config_file, get_defect_metadata, get_defect_anno_dict_train, get_defect_anno_dict_val
from analyze_maskrcnn_addmatrix import analyze_checkpoints, plot_learning_curve, plot_overall_stats_vs_iou_threshold, save_excel_together_finalreport

default_output_dir = "/ocean/projects/mat240020p/nliu3/maskrcnn/output/backup"
# input_dir='/jet/home/nliu3/cnn/figshare/MaskRCNNObjectDetection/code/train/ldm/'
...
def analyze_maskrcnn(cfg, defect_metadata, input_yaml, analyze_test=True, analyze_train=False):
    ...
    if os.path.exists(checkpoint_path):
        print("Checkpoint found, resuming training from", checkpoint_path)
    else:
    ...
    for iou_score_threshold_test in iou_score_threshold_tests:
        ...
        if (analyze_test == True and analyze_train == True):
            print("start to plot learning curve")
    ...
    print("start to plot_overall_stats_vs_iou_threshold")
    ...
    print("start to save excel together")
    ...
    save_excel_together_finalreport(full_dict_dfs_per_IoUscorethreshold=full_dict_dfs_per_IoUscorethreshold,
                                    ...
                                    save_path=os.path.join(cfg.OUTPUT_DIR, f'FinalReport_AllDefectDataAnalysis_{iou_score_threshold_tests}.xlsx'))
...
if __name__=='__main__':
    ...
    output_dir = input_yaml.get("output_dir", "default_output_dir")  # Use a default if key is missing
    os.makedirs(output_dir, exist_ok=True)
    with open(output_dir+"/config.yaml", "w") as config_file:
        yaml.dump(input_yaml, config_file)
