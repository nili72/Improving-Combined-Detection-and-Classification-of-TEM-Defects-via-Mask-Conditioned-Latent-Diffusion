...
def get_defect_size(segmentation, image_name, defect_type):
    ...
    #print('SEGMENTATION')
    #print(segmentation)
    # Check if segmentation is a list and has at least two elements
    if not isinstance(segmentation, list) or len(segmentation) < 2:
        print(f"Warning: segmentation for image {image_name} (defect type {defect_type}) is not in expected format: {segmentation}")
        # You can choose to return default values (e.g., 0, 0) or skip this annotation.
        return 0, 0

    # Now, depending on your annotation format you might have two cases.
    # Case 1: segmentation is a flat list of coordinates (e.g. [x1, y1, x2, y2, ...])
    if all(isinstance(val, (int, float)) for val in segmentation):
        # Separate x and y coordinates from the flat list.
        seg_x = segmentation[0::2]  # every even index
        seg_y = segmentation[1::2]  # every odd index
...
def get_defect_sizes_average_and_errors(true_defectsizes_bdot_nm, true_defectsizes_111_nm, true_defectsizes_100_nm,
    ...
    all_true_defects_flat = np.concatenate(
        [np.atleast_1d(x).flatten() for x in true_defectsizes_100_nm] +
        [np.atleast_1d(x).flatten() for x in true_defectsizes_111_nm] +
        [np.atleast_1d(x).flatten() for x in true_defectsizes_bdot_nm]
    )

    ax.hist(all_true_defects_flat, bins=np.arange(0, 100, 10), color='b', edgecolor='k', alpha=0.50, label='True')
    ...
    ax.set_ylabel('Number of instances', fontsize=12)
    ax2 = ax.twinx()
    ax2.plot(sorted(all_true_defects_flat), range(len(all_true_defects_flat)), 'b--', linewidth=1)
    ...
    if not all_true_defects:
        print("Warning: true_defectsizes_all_nm is empty.")
        true_stats = {
            "mean": 0, "std": 0, "min": 0, 
            "25%": 0, "50%": 0, "75%": 0, "max": 0, "skew": 0
        }
        ...
        # Convert to DataFrame and compute statistics
        true_stats = pd.DataFrame(all_true_defects_flat).describe().to_dict()[0]
    ...
    if not true_defectshapes_100:
        print("Warning: true_defectshapes_100 is empty.")
        true_stats = {
            "mean": 0, "std": 0, "min": 0, 
            "25%": 0, "50%": 0, "75%": 0, "max": 0, "skew": 0
        }
    ...
    fig, ax = get_fig_ax(aspect_ratio=0.5, x_align=0.65)
    all_true_shapes_flat = np.concatenate(
        [np.atleast_1d(x).flatten() for x in true_defectshapes_100] +
        [np.atleast_1d(x).flatten() for x in true_defectshapes_111] +
        [np.atleast_1d(x).flatten() for x in true_defectshapes_bdot]
    )

    ax.hist(all_true_shapes_flat, bins=np.arange(1, 2, 0.1), color='b', edgecolor='k', alpha=0.50, label='True')
    ...
    ax.set_ylabel('Number of instances', fontsize=12)
    ax2 = ax.twinx()
    ax2.plot(sorted(all_true_shapes_flat), range(len(all_true_shapes_flat)), 'b--', linewidth=1, label='True')
    ax2.set_ylabel('Total number of true defects', fontsize=12)
...
# --- Step 1: Define a function to extract all ground‐truth values from your annotations ---
def extract_ground_truth(anno_dict_list_val):
    """
    Loop over all annotation dictionaries and build complete lists for the ground‐truth defect sizes and shapes.
    
    Returns:
        gt_stats: a dictionary with keys:
            - 'true_defectsizes_bdot_nm_all'
            - 'true_defectsizes_111_nm_all'
            - 'true_defectsizes_100_nm_all'
            - 'true_defectshapes_bdot_all'
            - 'true_defectshapes_111_all'
            - 'true_defectshapes_100_all'
    """
    gt_stats = {
        'true_defectsizes_bdot_nm_all': [],
        'true_defectsizes_111_nm_all': [],
    ...
    # Loop over each image's annotation dictionary
    for anno in anno_dict_list_val:
        image_name = anno.get("file_name", "unknown")
        # Loop over each defect (annotation) in the image
        for obj in anno.get("annotations", []):
            segmentation = obj.get("segmentation", None)
            defect_type = obj.get("category_id", None)
            if segmentation is None or defect_type is None:
                continue  # Skip if essential data is missing

            # Compute defect size and shape using your predefined function.
            # (Make sure get_defect_size is defined in your code.)
            defect_diameter, defect_shape = get_defect_size(segmentation, image_name, defect_type)
            
            # Save the values in the appropriate lists based on defect type.
            if defect_type == 0:
                gt_stats['true_defectsizes_bdot_nm_all'].append(defect_diameter)
                gt_stats['true_defectshapes_bdot_all'].append(defect_shape)
            elif defect_type == 1:
                gt_stats['true_defectsizes_111_nm_all'].append(defect_diameter)
                gt_stats['true_defectshapes_111_all'].append(defect_shape)
            elif defect_type == 2:
                gt_stats['true_defectsizes_100_nm_all'].append(defect_diameter)
                gt_stats['true_defectshapes_100_all'].append(defect_shape)
    return gt_stats

# # --- Step 2: Call the function to extract the full ground‐truth data ---
# # (Assuming you already have your list of annotation dictionaries: anno_dict_list_val)
# gt_stats = extract_ground_truth(anno_dict_list_val)
...
def analyze_checkpoints(
    ...
    for model_checkpoint in model_checkpoints:
        ...
        if checkpoints_done < num_checkpoints:
            ...
            import os
            import pandas as pd
            output_path = os.path.join(cfg.OUTPUT_DIR, "Aggregated_TrueData_AllThresholds.xlsx")
            with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
                for true_and_pred_matching_threshold in true_and_pred_matching_thresholds:
                    ...
                    # Create a dictionary where each key maps to a Pandas Series.
                    # Using pd.Series ensures that even if the lists are of different lengths,
                    # you’ll get a column of the appropriate length (with NaNs for missing values).
                    all_data_dict = {
                        "true_defectsizes_bdot_nm_all": pd.Series(data['true_defectsizes_bdot_nm_all']),
                        "true_defectsizes_111_nm_all": pd.Series(data['true_defectsizes_111_nm_all']),
                        "true_defectsizes_100_nm_all": pd.Series(data['true_defectsizes_100_nm_all']),
                        "true_defectshapes_bdot_all": pd.Series(data['true_defectshapes_bdot_all']),
                        "true_defectshapes_111_all": pd.Series(data['true_defectshapes_111_all']),
                        "true_defectshapes_100_all": pd.Series(data['true_defectshapes_100_all']),
                        "pred_defectsizes_bdot_nm_all": pd.Series(data['pred_defectsizes_bdot_nm_all']),
                        "pred_defectsizes_111_all": pd.Series(data['pred_defectsizes_111_nm_all']),
                        "pred_defectsizes_100_nm_all": pd.Series(data['pred_defectsizes_100_nm_all']),
                        "pred_defectshapes_bdot_all": pd.Series(data['pred_defectshapes_bdot_all']),
                        "pred_defectshapes_111_all": pd.Series(data['pred_defectshapes_111_all']),
                        "pred_defectshapes_100_all": pd.Series(data['pred_defectshapes_100_all'])
                    }
                    # Create the DataFrame
                    df_all = pd.DataFrame(all_data_dict)

                    # Create a filename that encodes the thresholds        
                    # output_path = os.path.join(cfg.OUTPUT_DIR, "Aggregated_TrueData_AllThresholds.xlsx")
                    # with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
                    sheet_name = f"IOU_{iou_score_threshold_test}_Conf_{true_and_pred_matching_threshold}"
                    df_all.to_excel(writer, sheet_name=sheet_name, index=False)
                    print(f"Saved aggregated true/pred data workbook to {output_path}")
                    ...
                    data['true_defectsizes_bdot_nm_all'] = gt_stats['true_defectsizes_bdot_nm_all'],
                    data['true_defectsizes_111_nm_all'] = gt_stats['true_defectsizes_111_nm_all'],
                    data['true_defectsizes_100_nm_all'] = gt_stats['true_defectsizes_100_nm_all'],
                    
                    data['true_defectshapes_bdot_all'] = gt_stats['true_defectshapes_bdot_all'],  
                    data['true_defectshapes_111_all'] = gt_stats['true_defectshapes_111_all'],
                    data['true_defectshapes_100_all'] = gt_stats['true_defectshapes_100_all'],                    
                        # use independent ground truth
