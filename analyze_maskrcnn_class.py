...
def get_true_data_stats(cfg, defect_metadata, anno_dict_list_val, filename, model_checkpoint, true_and_pred_matching_threshold,
    ...
    if mask_on == True:
        ...
        #     cv2_imshow(img2)
        if save_images == True:
            cv2.imwrite(os.path.join(cfg.OUTPUT_DIR, filename + '_true_'+'TruePredMatch_'+'.png'), img2)
...
def get_pred_data_stats(cfg, defect_metadata, anno_dict_list_val, filename, predictor, model_checkpoint,
    ...
    #     cv2.imshow(img3)
    if save_images == True:
        cv2.imwrite(os.path.join(cfg.OUTPUT_DIR, filename + '_predicted_'+'TruePredMatch_'+
...
def analyze_checkpoints(cfg, defect_metadata, input_yaml, iou_score_threshold_test, test_dataset_path,
    ...
    '''
      # Read values from YAML if true_and_pred_matching_thresholds is not provided
    if true_and_pred_matching_thresholds is None:
        try:
            # Check if input_yaml is a dictionary or a file path
            if isinstance(input_yaml, dict):
                # If it's already a dictionary, use it directly
                yaml_data = input_yaml
            else:
                # Otherwise, assume it's a path and read the YAML file
                with open(input_yaml, 'r') as yaml_file:
                    yaml_data = yaml.safe_load(yaml_file)

            # Retrieve the thresholds or use the default
            true_and_pred_matching_thresholds = yaml_data.get('true_and_pred_matching_thresholds', [0.7])

            # Validate that the thresholds are a list
            if not isinstance(true_and_pred_matching_thresholds, list):
                raise ValueError("true_and_pred_matching_thresholds must be a list in the YAML file or input dictionary.")
        except Exception as e:
            ...
            for true_and_pred_matching_threshold in true_and_pred_matching_thresholds:
                ...
                for filename in validation_image_filenames:
                    if images_done < num_images:
                        ...
                        try:
                            results = get_pred_data_stats(cfg=cfg, defect_metadata=defect_metadata,
                                                                anno_dict_list_val=anno_dict_list_val,
                            ...
                            if results is None:
                                print(f"Skipping filename {filename}: No valid data returned from get_pred_data_stats.")
                                continue  # Skip to the next iteration or image if applicable
                            pred_pixels_all_oneimage, pred_classes_all_oneimage, pred_segmentations_oneimage, \
                            pred_boxes_oneimage = results
                        ...
                        if len(pred_segmentations_oneimage) == 0:
                            print('No predictions found for image', filename)
                            continue

                        if len(pred_segmentations_oneimage) != len(pred_classes_all_oneimage):
                            print('Found problem where number of predicted segs does not match number of preds')
                            continue
                        ...
                        if not (true_segmentations_oneimage and true_classes_all_oneimage and true_boxes_oneimage):
                            print("One or more input lists are empty:")
                            print(f"true_segmentations_oneimage: {true_segmentations_oneimage}")
                            print(f"true_classes_all_oneimage: {true_classes_all_oneimage}")
                            print(f"true_boxes_oneimage: {true_boxes_oneimage}")
                            true_segmentations_oneimage_sorted = []
                            true_classes_all_oneimage_sorted = []
                            true_boxes_oneimage_sorted = []
                        else:
                ...
                if true_avg_size_bd_perimage_list and pred_avg_size_bd_perimage_list:
                    ...
                    print("Warning: true_avg_size_bd_perimage_list is empty. Defaulting xlow and xhigh.")
                    # Provide default values or handle appropriately
                    xlow = 0  # Default value or logic
                    xhigh = 0  # Default value or logic
                    if image_name_list_bd_size:
                        true_avg_size_bd_perimage_list = [0]* len(image_name_list_bd_size)
                        pred_avg_size_bd_perimage_list = [0]* len(image_name_list_bd_size)
                    else: 
                        true_avg_size_bd_perimage_list = pred_avg_size_bd_perimage_list = image_name_list_bd_size = [0]
                # print(f"Length of true_avg_size_bd_perimage_list: {len(true_avg_size_bd_perimage_list)}")
                # print(f"Length of pred_avg_size_bd_perimage_list: {len(pred_avg_size_bd_perimage_list)}")
                # print(f"Length of image_name_list_bd_size: {len(image_name_list_bd_size)}")
                ...
                try:
                    ...
                    for key, value in data_dict.items():
                        if len(value) < max_length:
                            data_dict[key].extend([None] * (max_length - len(value)))

                    # Create the DataFrame and save it to Excel
                    pd.DataFrame.from_dict(data=data_dict).to_excel(
                        os.path.join(cfg.OUTPUT_DIR, 'DefectSize_ParityPlot_TruePredMatch_' + str(true_and_pred_matching_threshold) +
                                    '_IoUScoreThresh_' + str(iou_score_threshold_test) + '_Blackdot' + '.xlsx'),
                        index=False
                    )
                except ValueError as e:
                    print(f"Error: Mismatched data lengths in data_dict. Details: {e}")
                except Exception as e:
                ...
                if true_avg_size_111_perimage_list and pred_avg_size_111_perimage_list:
                    ...
                    xlow = 0  # Default value or logic
                    xhigh = 0  # Default value or logic
                    print("Warning: true_avg_size_111_perimage_list is empty. Defaulting xlow and xhigh.")
                    if image_name_list_111_size:
                        true_avg_size_111_perimage_list = [0]* len(image_name_list_111_size)
                        pred_avg_size_111_perimage_list = [0]* len(image_name_list_111_size)
                    else: 
                        true_avg_size_111_perimage_list = pred_avg_size_111_perimage_list = image_name_list_111_size = [0]
                ...
                if true_avg_size_100_perimage_list and pred_avg_size_100_perimage_list:
                    ...
                    xlow = 0  # Default value or logic
                    xhigh = 0  # Default value or logic
                    print("Warning: true_avg_size_100_perimage_list is empty. Defaulting xlow and xhigh.")
                    if image_name_list_100_size:
                        true_avg_size_100_perimage_list = [0]* len(image_name_list_100_size)
                        pred_avg_size_100_perimage_list = [0]* len(image_name_list_100_size)
                    else: 
                        true_avg_size_100_perimage_list = pred_avg_size_100_perimage_list = image_name_list_100_size = [0]
                # xlow = int(min(true_avg_size_100_perimage_list) - 0.1*(max(true_avg_size_100_perimage_list)-min(true_avg_size_100_perimage_list)))
                # xhigh = int(max(true_avg_size_100_perimage_list) + 0.1*(max(true_avg_size_100_perimage_list)-min(true_avg_size_100_perimage_list)))
                ax.set_xlim(left=xlow, right=xhigh)
                ax.set_ylim(bottom=xlow, top=xhigh)
                ...
                if all_avg_true_sizes and all_avg_pred_sizes:
                    ...
                    xlow = 0  # Default value or logic
                    xhigh = 0  # Default value or logic
                    all_avg_true_sizes = [0]
                    all_avg_pred_sizes = [0]
                ...
                if true_density_bd_perimage_list and  pred_density_bd_perimage_list:
                    ...
                    xlow = 0  # Default value or logic
                    xhigh = 0  # Default value or logic
                    print("Warning: true_density_bd_perimage_list is empty. Defaulting xlow and xhigh.")
                    if image_name_list_bd_density:
                        true_density_bd_perimage_list = [0]*len(image_name_list_bd_density)
                        pred_density_bd_perimage_list = [0] *len(image_name_list_bd_density)  
                    else: 
                        true_density_bd_perimage_list = pred_density_bd_perimage_list = image_name_list_bd_density = [0]
                # xlow = int(min(true_density_bd_perimage_list) - 0.1*(max(true_density_bd_perimage_list)-min(true_density_bd_perimage_list)))
                # xhigh = int(max(true_density_bd_perimage_list) + 0.1*(max(true_density_bd_perimage_list)-min(true_density_bd_perimage_list)))
                #xlow = 0.0
                #xhigh = 10.0
                ...
                if true_density_111_perimage_list and pred_density_111_perimage_list:
                    ...
                    xlow = 0  # Default value or logic
                    xhigh = 0  # Default value or logic
                    print("Warning: true_density_111_perimage_list is empty. Defaulting xlow and xhigh.")
                    if image_name_list_111_density:
                        true_density_111_perimage_list = [0]*len(image_name_list_111_density)
                        pred_density_111_perimage_list = [0] *len(image_name_list_111_density)  
                    else: 
                        true_density_111_perimage_list = pred_density_111_perimage_list = image_name_list_111_density = [0]
                # xlow = int(min(true_density_111_perimage_list) - 0.1*(max(true_density_111_perimage_list)-min(true_density_111_perimage_list)))
                # xhigh = int(max(true_density_111_perimage_list) + 0.1*(max(true_density_111_perimage_list)-min(true_density_111_perimage_list)))
                #xlow = 0.0
                #xhigh = 10.0
                ...
                if true_density_100_perimage_list and pred_density_100_perimage_list:
                    ...
                    xlow = 0  # Default value or logic
                    xhigh = 0  # Default value or logic
                    print("Warning: true_density_100_perimage_list is empty. Defaulting xlow and xhigh.")
                    if image_name_list_100_density:
                        true_density_100_perimage_list = [0]*len(image_name_list_100_density)
                        pred_density_100_perimage_list = [0] *len(image_name_list_111_density)  
                    else: 
                        true_density_100_perimage_list = pred_density_100_perimage_list = image_name_list_100_density = [0]
                # xlow = int(min(true_density_100_perimage_list) - 0.1*(max(true_density_100_perimage_list)-min(true_density_100_perimage_list)))
                # xhigh = int(max(true_density_100_perimage_list) + 0.1*(max(true_density_100_perimage_list)-min(true_density_100_perimage_list)))
                #xlow = 0.0
                #xhigh = 10.0
                ...
                if all_true_densities and all_pred_densities:
                    ...
                    print("Warning: all_true_densities is empty. Defaulting xlow and xhigh.")
                    # Provide default values or handle appropriately
                    xlow = 0  # Default value or logic
                    xhigh = 0  # Default value or logic   
                    all_true_densities = [0]
                    all_pred_densities = [0]  
                # xlow = int(min(all_true_densities) - 0.1*(max(all_true_densities)-min(all_true_densities)))
                # xhigh = int(max(all_true_densities) + 0.1*(max(all_true_densities)-min(all_true_densities)))
                #xlow = 0.0
                #xhigh = 10.0
                ...
                if not (true_avg_shape_bd_perimage_list and pred_avg_shape_bd_perimage_list):
                    print("Warning: true_avg_shape_bd_perimage_list is empty. Defaulting xlow and xhigh.")
                    # Provide default values or handle appropriately
                    if image_name_list_bd_shape:
                        true_avg_shape_bd_perimage_list = [0]* len(image_name_list_bd_shape)
                        pred_avg_shape_bd_perimage_list = [0]* len(image_name_list_bd_shape)
                    else: 
                        true_avg_shape_bd_perimage_list = pred_avg_shape_bd_perimage_list = image_name_list_bd_shape = [0]
                ...
                if not(true_avg_shape_111_perimage_list and pred_avg_shape_111_perimage_list):
                    print("Warning: true_avg_shape_111_perimage_list is empty. Defaulting xlow and xhigh.")
                    # Provide default values or handle appropriately
                    if image_name_list_111_shape:
                        true_avg_shape_111_perimage_list = [0]* len(image_name_list_111_shape)
                        pred_avg_shape_111_perimage_list = [0]* len(image_name_list_111_shape)
                    else: 
                        true_avg_shape_111_perimage_list = pred_avg_shape_111_perimage_list = image_name_list_111_shape = [0]             
                ...
                if not(true_avg_shape_100_perimage_list and pred_avg_shape_100_perimage_list):
                    print("Warning: true_avg_shape_100_perimage_list is empty. Defaulting xlow and xhigh.")
                    # Provide default values or handle appropriately
                    if image_name_list_100_shape:
                        true_avg_shape_100_perimage_list = [0]* len(image_name_list_100_shape)
                        pred_avg_shape_100_perimage_list = [0]* len(image_name_list_100_shape)
                    else: 
                        true_avg_shape_100_perimage_list = pred_avg_shape_100_perimage_list = image_name_list_100_shape = [0] 
                ...
                ax.set_xlim(left=xlow, right=xhigh)
                ax.set_ylim(bottom=xlow, top=xhigh)
                if not (all_true_avg_shapes and all_pred_avg_shapes):
                    all_true_avg_shapes = all_pred_avg_shapes = [0] 
                ...
                best_f1 = -float('inf')  # Initialize to negative infinity for maximization

                for img, data in data_dict_per_image.items():
                    if data.get('avg size error',float('inf')) < best_size:
                        best_size = data['avg size error']
                        data_dict_best_images['best avg size error'] = img

                    if data.get('avg density error', float('inf')) < best_density:
                        best_density = data['avg density error']
                        data_dict_best_images['best avg density error'] = img

                    if data.get('overall F1', -float('inf')) > best_f1:
                        best_f1 = data['overall F1']
                        data_dict_best_images['best overall F1'] = img

                    if data.get('defect find F1', -float('inf')) > best_find_f1:
                        best_find_f1 = data['defect find F1']
                        data_dict_best_images['best defect find F1'] = img
                ...
                for img, data in data_dict_per_image.items():
                    if data.get('avg size error', 0) > worst_size:
                        worst_size = data['avg size error']
                        data_dict_worst_images['worst avg size error'] = img
                    if data.get('avg density error', 0) > worst_density:
                        worst_density = data['avg density error']
                        data_dict_worst_images['worst avg density error'] = img
                    if data.get('overall F1', float('inf')) < worst_f1:
                        worst_f1 = data['overall F1']
                        data_dict_worst_images['worst overall F1'] = img
                    if data.get('defect find F1', float('inf')) < worst_find_f1:
                        worst_find_f1 = data['defect find F1']
                        data_dict_worst_images['worst defect find F1'] = img
